  ...
  return (
    <div className="max-w-6xl mx-auto pb-10 space-y-6">
      <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="flex items-center justify-between">
        <Link href={`/siswa/${id}`} className="inline-flex items-center gap-2 text-sm text-slate-400 hover:text-white transition-colors">
          <ArrowLeft className="w-4 h-4" /> Batal Edit
        </Link>
      </motion.div>
      <SiswaForm initialData={siswa} mode="edit" />
    </div>
  );
}
